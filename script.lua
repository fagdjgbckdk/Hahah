...
local names={"Fly","Invisible","NoClip"}local sts={false,false,false}
local bv=Instance.new("BodyVelocity",hrp)bv.MaxForce=Vector3.one*1e9 bv.P=1250 bv.Velocity=Vector3.zero bv.Enabled=false
...
if i==2 then for _,v in pairs(c:GetDescendants())do
if v:IsA("BasePart")and v.Name~="HumanoidRootPart"or v:IsA("Decal")then
v.Transparency=sts[i]and 1 or 0 end end end end)end

game:GetService("RunService").RenderStepped:Connect(function()
if sts[1]then local cf=workspace.CurrentCamera.CFrame local mv=Vector3.zero
...
if uis:IsKeyDown(Enum.KeyCode.Space)then mv+=cf.UpVector end
if uis:IsKeyDown(Enum.KeyCode.LeftControl)then mv-=cf.UpVector end
...
p.CharacterAdded:Connect(function(chr)c=chr hrp=c:WaitForChild("HumanoidRootPart")
bv=Instance.new("BodyVelocity",hrp)bv.MaxForce=Vector3.one*1e9 bv.P=1250 bv.Velocity=Vector3.zero end)
